...
for i, batch in enumerate(im_batches):
	...
	if type(prediction) == int:
		for im_num, image in enumerate(imlist[i*batch_size: min((i+1)*batch_size, len(imlist))]):
			...
			print("{0:20s} predicted in {1:6.3f} seconds".format(images.split("/")[-1],
																	(end-start)/batch_size))
			print("{0:20s} {1:s}".format("Objects Detected:", ""))
			print("-----------------------------------------------------------")
		continue
	
	prediction[:,0] += i*batch_size #transform the atribute from index in batch to index in imlit
...
print("SUMMARY")
print("----------------------------------------------")
print("{:25s}: {}\n".format("Task", "Time Taken (in seconds)"))
